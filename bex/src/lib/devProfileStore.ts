...
export function getAllDevProfiles(): Array<{ uid: string; profile: Partial<BexUser> }> {
  const result: Array<{ uid: string; profile: Partial<BexUser> }> = [];
  profiles.forEach((profile, uid) => {
    result.push({ uid, profile });
  });
  return result;
}
